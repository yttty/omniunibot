...
def test_config_2():
    ...
    cfg_dict_full = {
        ...
        "log": {
            "level": "DEBUG",
            "dir": str(Path.home() / "logs" / "omniunibot"),
        },
        "channel_groups": {
            "test_channels": [
                {
    ...
    cfg = OmniUniBotConfig.from_dict(cfg_dict_abbr)
    assert cfg == OmniUniBotConfig.from_dict(cfg_dict_full)
    assert cfg.to_dict() == cfg_dict_full
